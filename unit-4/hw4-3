...
def csuffix(X,Y):
    ...
    if X is "" or Y is "" : return 0
    if (X,Y) in chart : return chart[X,Y]
    suffsize = 0
    #print X,Y
    if X[-1:] == Y[-1:] :
        suffsize = 1 + csuffix(X[:-1],Y[:-1])
    chart[X,Y] = suffsize    
    return suffsize

#print csuffix("Tapach","Temapach")
#print chart[("Tapach","Temapach")]
#print max([5,3,8,6])


def prefixes(X):
    return [X[:index+1] for index in range(len(X))]
    # Fill in your code here (can be done in one or two lines). 

#print prefixes("Tapach")
