...
def heistvalid(available, limit, taken):
    return sum([available.values()[index][0] for item in taken for index in range(len(available)) if available.keys()[index] == item ]) <= limit
    # Replace this line with your one-line answer.



def heisttotal(available, limit, taken):
    return sum([available.values()[index][1] for item in taken for index in range(len(available)) if available.keys()[index] == item ])
    # Replace this line with your one-line answer.
