...
def cfginfinite(grammar):
    for token in [rule[0] for rule in grammar ]:
        def recurs(current, visited, sizexy):
            if current in visited: return sizexy > 0
            else :
                new_visited = visited + [current]
                for new_symbols in [rule[1] for rule in grammar if rule[0] == current]:
                    for symbol in new_symbols:
                        if recurs(symbol, new_visited, sizexy + len(new_symbols) - 1):
                            return True
                return False
            
        if recurs(token,[],0): return True
    return False    
    # Put your code here!
