...
def cfgempty(grammar,symbol,visited):
    ...
    if symbol in visited:
        ...
        for new_symbols in [r[1] for r in grammar if r[0] == symbol]:
            if all([None != cfgempty(grammar,letter,new_visited) for letter in new_symbols]):
                result = []
                for letter in new_symbols:
                    result = result + cfgempty(grammar,letter,new_visited)
                return result
    return None
