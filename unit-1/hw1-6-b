...
def nfsmaccepts(current, edges, accepting, visited): 
        ...
        if len(visited) == 0 : visited.append((-1, 'xy'))
        
        if current == accepting[0] : return ""
        elif str(current) not in list(re.findall('\d',str(edges.keys()))) : return None
        else :
            for index in range(len(edges)) :
                #print (index, edges.keys()[index][0], current, edges.keys()[index], visited[len(visited)-2])
                if edges.keys()[index][0] == current :
                    visited.append(edges.keys()[index])
                    if visited[len(visited)-2][0] == edges.keys()[index][0] : return False
                    else :
                        string = edges.keys()[index][1]
                        destination = edges.values()[index]
                        
                        for index2 in range(len(destination)) :
                            #print (string, visited[len(visited)-2], destination, index2)
                            letter = nfsmaccepts(destination[index2], edges, accepting, visited)
                            if letter == False and len(visited) == 3 : return None
                            elif letter == False : string = ""
                            else : string += letter
                            #print string
        return string
