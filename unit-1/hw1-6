    ...
    if current in accepting:
        ...
        for edge in edges:
            for destination in edges[edge]:
                if destination not in visited and edge[0] == current:
                    letter = nfsmaccepts(destination, edges, accepting, visited)
                    if letter is not None:
                        string.append(edge[1] + letter)
        return None if not string else string[0]
