...
def reverse(edges,accepting,start):
    ...
    new_edges = dict([((dest, letter),[current]) for (current, letter) in edges \
                           for dest in edges[(current, letter)]])
    
    for (current, letter) in edges:
        for dest in edges[(current, letter)]:
            if current not in new_edges[(dest, letter)] :
                new_edges[(dest, letter)].append(current)
